...
from dashboard_api.infrastructure.persistence.sqlite_database import (
    CURRENT_SCHEMA_VERSION,
    SqliteDatabase,
)
from dashboard_api.infrastructure.preferences.json_store import JsonPreferencesStore
from dashboard_api.shared.time import utc_now, utc_now_iso
...
class DashboardBackupService:
    ...
    def __init__(
        ...
        *,
        ...
        preferences_service: DashboardPreferencesService,
    ...
    def export_backup(self, destination_path: Path) -> Path:
        ...
        with TemporaryDirectory(
            ...
            preferences_snapshot.write_text(
                json.dumps(
                    asdict(self._preferences_service.get_preferences()),
                    indent=2,
                    sort_keys=True,
                ),
                encoding="utf-8",
    ...
    def _import_backup_locked(self, source_path: Path) -> BackupImportResult:
        ...
        with TemporaryDirectory(
            ...
            self._preferences_service.reload_preferences()
    ...
    def _replace_local_files(
        ...
        rollback_preferences = self._preferences_path.with_suffix(
            ".restore-rollback",
        )
        ...
        if self._preferences_path.exists():
            shutil.copy2(self._preferences_path, rollback_preferences)
        try:
            shutil.copy2(imported_database_path, self._database.path)
            shutil.copy2(imported_preferences_path, self._preferences_path)
            ...
            if rollback_preferences.exists():
                shutil.copy2(rollback_preferences, self._preferences_path)
            raise
        finally:
            rollback_database.unlink(missing_ok=True)
            rollback_preferences.unlink(missing_ok=True)
