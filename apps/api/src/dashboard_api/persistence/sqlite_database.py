...
def _migration_3(connection: sqlite3.Connection) -> None:
    """Persist the local workflow independently of GitHub issue state."""
    connection.execute(
        "ALTER TABLE tracked_issues "
        "ADD COLUMN local_status TEXT NOT NULL DEFAULT 'active' "
        "CHECK (local_status IN ('active', 'in_review', 'completed'))",
    )
    connection.execute(
        "UPDATE tracked_issues SET local_status = 'completed' "
        "WHERE local_completed_at IS NOT NULL",
    )
